                ...
                <div key={index} className="relative flex items-center group">
                  ...
                  >
                    <div className={`flex ${isLeft ? 'justify-end' : 'justify-start'}`}>
                      <div className="fade-in-up w-full max-w-md rounded-lg bg-card p-6 shadow-md transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-accent/20">
                        <p className="text-sm text-muted-foreground">{item.period}</p>
                        <h3 className="mt-1 text-xl font-bold text-primary">{item.role}</h3>
                        <p className="mt-1 font-semibold">{item.company}</p>
                        <ul className="mt-2 text-muted-foreground list-disc list-inside text-left">
